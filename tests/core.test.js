...
test('analysis span supports exact 10-minute fractional hours',()=>{const p=trajectory([18.5,-69.9],start,1.5,hourly);assert.equal(p.points.length,10);assert.equal(p.times.at(-1),start+90*60000);assert.throws(()=>trajectory([18.5,-69.9],start,1.01,hourly),/10 minutos/);});
